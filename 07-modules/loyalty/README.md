...
Owns tenant loyalty program configuration, tiers, customer memberships, and immutable loyalty points ledger.
...
| --- | --- | --- | --- |
| `loyalty_programs` | `id` | tenant_id -> tenants.id; code | Tenant loyalty program config. |
| `membership_tiers` | `id` | tenant_id -> tenants.id; loyalty_program_id | Tier definitions. |
| `customer_memberships` | `id` | tenant_id -> tenants.id; customer_id; loyalty_program_id; tier_id | Membership and points projection. |
| `loyalty_transactions` | `id` | tenant_id -> tenants.id; customer_membership_id; source sale/order/return | Immutable loyalty ledger. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/loyalty/features/loyalty-programs/feature-spec|Loyalty Programs]] | Tenant-owned loyalty program configuration with JSON earn/redeem rules. | `loyalty_programs` |
| [[07-modules/loyalty/features/membership-tiers/feature-spec|Membership Tiers]] | Tier definitions inside a loyalty program. | `membership_tiers`, `loyalty_programs` |
| [[07-modules/loyalty/features/customer-memberships/feature-spec|Customer Memberships]] | Customer membership record and current points projection. | `customer_memberships`, `customers`, `loyalty_programs`, `membership_tiers` |
| [[07-modules/loyalty/features/loyalty-transactions/feature-spec|Loyalty Transactions]] | Immutable loyalty points ledger for earn, redeem, adjust, expire, and reverse. | `loyalty_transactions`, `customer_memberships`, `sales`, `orders`, `returns` |
