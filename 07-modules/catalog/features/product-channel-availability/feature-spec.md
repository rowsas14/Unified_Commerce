---
title: Product Channel Availability Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, product-channel-availability]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Product Channel Availability Feature Spec

## Purpose

Controls whether products are sellable in POS, e-commerce, or both channels through documented product fields.
...
| Actor | Usage |
|---|---|
| Tenant Admin / Catalog Manager | Primary actor for this feature. |
...
| Table | Usage |
|---|---|
| `products` | Uses `is_sellable_pos`, `is_sellable_online`, `status`, `product_type`, `track_inventory`. |
| `product_variants` | Variant status must allow selling for channel workflows. |
| `price_lists` | Channel-aware pricing source: `pos`, `ecommerce`, or `both`. |
| `inventory_channel_allocations` | Optional channel stock allocation belongs to Inventory but affects availability. |
...
1. Edit product channel flags for POS and/or e-commerce.
2. Validate product status and active variant availability.
3. Ensure pricing exists for the intended channel before selling if required by pricing rules.
4. Use POS flag for cashier search and scan eligibility.
5. Use e-commerce flag for online listing/cart eligibility.

## Validation rules

- Archived products must not be newly sold through either channel.
- POS-only products must not appear in e-commerce cart/checkout.
- E-commerce-only products must not appear as sellable POS scan results unless explicitly allowed by product flags.
- Channel-specific stock allocation, when used, must not exceed available stock.
