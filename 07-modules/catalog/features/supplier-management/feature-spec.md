---
title: Supplier Management Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, supplier-management]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Supplier Management Feature Spec

## Purpose

Maintains supplier master data, addresses, and product/variant supplier mappings for stock receiving readiness.
...
| Table | Usage |
|---|---|
| `suppliers` | PK `id`; FK `tenant_id`; owns supplier name, contact, email, phone, status. |
| `supplier_addresses` | PK `id`; FK `tenant_id`, `supplier_id`; stores supplier address and primary flag. |
| `product_suppliers` | PK `id`; FK `tenant_id`, `product_id`, optional `variant_id`, `supplier_id`; stores supplier SKU, purchase price, lead time, primary flag, status. |
| `purchase_receipts` | Inventory receiving document can reference supplier; owned by Inventory module. |
...
1. Create supplier under tenant with contact details and status.
2. Add one or more supplier addresses and mark one primary where applicable.
3. Map supplier to product or variant with supplier SKU and purchase details.
4. Use supplier references during purchase receiving and inventory replenishment workflows.

## Validation rules

- Supplier and address must belong to same tenant.
- At most one primary supplier address per supplier should be active.
- Product/variant/supplier mapping must be tenant-consistent.
- At most one primary supplier per product/variant should be active.
