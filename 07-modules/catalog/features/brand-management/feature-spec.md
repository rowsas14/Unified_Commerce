---
title: Brand Management Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, brand-management]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Brand Management Feature Spec

## Purpose

Maintains optional tenant-owned brand master data for products.
...
| Table | Usage |
|---|---|
| `brands` | PK `id`; FK `tenant_id`; owns brand name, optional code, status. |
| `products` | Optional FK `brand_id` references brands. |
...
1. Create brand under tenant.
2. Assign name, optional code, and active/inactive status.
3. Use active brand during product creation and filtering.
4. Keep existing product history stable if a brand becomes inactive.

## Validation rules

- Brand name must be unique per tenant.
- Brand code must be unique per tenant when present.
- Inactive brands should not be offered for new product assignment unless business permits through admin action.
- Brand must not be shared across tenants.
