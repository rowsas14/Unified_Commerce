---
title: Product Return Policy Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, product-return-policy]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Product Return Policy Feature Spec

## Purpose

Maintains return/exchange policy configuration assigned to products.
...
| Actor | Usage |
|---|---|
| Catalog Manager / Tenant Admin | Primary actor for this feature. |
...
| Table | Usage |
|---|---|
| `return_policies` | PK `id`; FK `tenant_id`; owns code, name, policy type, return/exchange windows, receipt requirement, damaged return allowance, manager override requirement, active flag. |
| `products` | Required FK `return_policy_id` determines product return behavior. |
| `returns` | Return execution belongs to Returns and Exchanges module. |
| `exchanges` | Exchange execution belongs to Returns and Exchanges module. |
...
1. Create or update return policy under tenant.
2. Set policy type: returnable, non-returnable, exchange-only, or final sale.
3. Set return/exchange windows and receipt/manager override requirements.
4. Assign policy to products.
5. Return/exchange workflows read policy during eligibility validation.

## Validation rules

- Return policy code must be unique per tenant.
- Products require a return policy reference.
- Non-returnable/final-sale policy must be enforced by returns workflow unless manager override is allowed.
- Policy changes must not rewrite historical sale/order facts.
