---
title: Product Pricing Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, product-pricing]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Product Pricing Feature Spec

## Purpose

Maintains tenant price lists and variant price entries used by POS and e-commerce channels.
...
| Actor | Usage |
|---|---|
| Catalog Manager / Pricing Manager | Primary actor for this feature. |
...
| Table | Usage |
|---|---|
| `price_lists` | PK `id`; FK `tenant_id`; owns name, channel, currency, effective period, priority, active flag. |
| `price_list_items` | PK `id`; FK `tenant_id`, `price_list_id`, `variant_id`, optional `outlet_id`; owns list price and optional sale price. |
| `product_variants` | Prices are assigned to variants, not product masters. |
| `outlets` | Optional outlet-specific price override through `outlet_id`. |
...
1. Create a price list for POS, e-commerce, or both channels.
2. Set currency, effective dates, priority, and active state.
3. Add variant prices with list price and optional sale price.
4. Optionally set outlet-specific override price entries.
5. Pricing resolver selects the correct price by tenant, channel, currency, date, priority, variant, and outlet.

## Validation rules

- Price list name must be unique per tenant.
- Price list item must reference a variant under the same tenant.
- List price and sale price must not be negative.
- Active overlapping price lists with same tenant/channel/currency/priority must be blocked or resolved by deterministic service rule.
- Do not store final transaction price only in price lists; sale/order lines freeze pricing snapshot.
