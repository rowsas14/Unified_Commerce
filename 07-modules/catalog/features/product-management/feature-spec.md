---
title: Product Management Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, product-management]
...
Creates and maintains the tenant-owned product master shared by POS and e-commerce.
...
| Table | Usage |
|---|---|
| `products` | PK `id`; FK `tenant_id`, `category_id`, `brand_id`, `tax_class_id`, `return_policy_id`; owns product type, name, slug, descriptions, status, sellable flags, inventory tracking flag. |
| `categories` | Validates optional product category within the same tenant. |
| `brands` | Validates optional product brand within the same tenant. |
| `tax_classes` | Validates product tax class reference; calculation belongs to Tax module. |
| `return_policies` | Required FK in `products`; determines return/exchange eligibility rules. |
...
1. Open product management from an allowed tenant context.
2. Create or edit product master data: category, brand, tax class, return policy, name, slug, descriptions, product type, status, and channel sellable flags.
3. Validate tenant ownership for every referenced category, brand, tax class, and return policy.
4. Save product as `draft`, `active`, or `archived` based on uploaded database status model.
5. Create or maintain variants through Product Variant Management before the item becomes sellable.
6. Expose active POS/e-commerce-eligible records to search, POS billing, cart, orders, returns, and reporting.

## Validation rules

- `tenant_id` is mandatory and cannot be changed across tenants.
- `slug` must be unique per tenant.
- `return_policy_id` is required.
- Do not allow product master save to create stock quantity.
- Do not allow selling workflows to use archived products.
