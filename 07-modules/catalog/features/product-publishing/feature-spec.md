---
title: Product Publishing Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, product-publishing]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Product Publishing Feature Spec

## Purpose

Defines readiness for showing products in the e-commerce channel using existing product status, sellable flags, descriptions, images, variants, and pricing data.
...
| Table | Usage |
|---|---|
| `products` | Uses `status`, `is_sellable_online`, descriptions, slug, tax class, return policy. |
| `product_variants` | Active variants required for sellable product types. |
| `product_images` | Online display images where available. |
| `price_lists` | E-commerce/both channel price source. |
| `price_list_items` | Variant price entries for online checkout. |
...
1. Prepare product details, variants, images, return policy, tax class, and e-commerce sellable flag.
2. Validate active status and online channel eligibility.
3. Validate price availability for e-commerce/both channel where required.
4. Expose product to e-commerce listing and cart only after backend validation.
5. Keep POS availability independent through POS sellable flag.

## Validation rules

- Do not create undocumented publication tables; current schema uses product status and channel flags.
- Online products should have usable descriptions/images when needed by product owner guidance.
- Cart and checkout must revalidate price and stock.
- Publishing must not bypass tenant feature access or product status rules.
