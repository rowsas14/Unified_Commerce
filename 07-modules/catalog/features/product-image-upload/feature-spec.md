---
title: Product Image Upload Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, product-image-upload]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Product Image Upload Feature Spec

## Purpose

Stores product and optional variant image references for online catalog display and POS item recognition.
...
| Table | Usage |
|---|---|
| `product_images` | PK `id`; FK `tenant_id`, `product_id`, optional `variant_id`; owns storage key, alt text, sort order, primary flag, created time. |
| `products` | Parent product must belong to tenant. |
| `product_variants` | Optional variant-level image target must belong to product/tenant. |
...
1. Select product or variant under tenant.
2. Upload or attach image using object storage key produced by the storage process.
3. Set alt text, sort order, and primary image flag.
4. Use image references in e-commerce product views and optional POS product tiles.
5. Keep image metadata tenant-scoped and auditable.

## Validation rules

- `storage_key` is required; binary storage details are not owned by the catalog table.
- Product and optional variant must belong to same tenant.
- At most one primary product-level image and one primary variant-level image should be active.
- Images must not decide product sellability; sellability belongs to product/channel/status rules.
