---
title: Product Attribute Management Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, product-attribute-management]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Product Attribute Management Feature Spec

## Purpose

Defines tenant-owned product attributes, selectable values, platform templates, presets, category mapping, and variant values.
...
| Table | Usage |
|---|---|
| `product_attributes` | PK `id`; FK `tenant_id`; owns code, name, data type, variant-defining flag, status. |
| `attribute_values` | PK `id`; FK `tenant_id`, `attribute_id`; stores value code/text, sort order, active flag. |
| `attribute_templates` | Platform-owned reusable attribute templates; no tenant_id. |
| `attribute_template_values` | Default selectable values under platform templates. |
| `attribute_presets` | Platform-owned attribute bundles by business type. |
| `attribute_preset_items` | Maps templates into presets. |
| `category_attributes` | Maps tenant categories to tenant attributes. |
| `variant_attribute_values` | Stores selected attribute values for variants. |
...
1. Create tenant-owned attributes or copy from platform templates/presets where available.
2. Create selectable values for attributes with `select` data type.
3. Mark attributes as variant-defining when they form SKU/variant dimensions.
4. Map required attributes to categories.
5. Assign selected values to variants during variant management.

## Validation rules

- Attribute code must be unique per tenant.
- Attribute value code must be unique per tenant and attribute.
- Platform templates are accelerators only; tenant-owned final attributes are stored in `product_attributes`.
- `attribute_value_id` must belong to the selected `attribute_id`.
