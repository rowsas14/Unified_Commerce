---
title: Product Variant Management Feature Spec
owner: Catalog Module Owner
status: production-ready
last_reviewed: 2026-04-30
tags: [feature-spec, catalog, product-variant-management]
source: uploaded-scope uploaded-database backend-architecture frontend-architecture
---

# Product Variant Management Feature Spec

## Purpose

Manages the sellable SKU and barcode units under products.
...
| Table | Usage |
|---|---|
| `product_variants` | PK `id`; FK `tenant_id`, `product_id`; owns SKU, barcode, variant name, weight, purchase limits, variant signature, status. |
| `variant_attribute_values` | Maps variant to selected attribute values. |
| `product_attributes` | Defines tenant-owned attributes used by variants. |
| `attribute_values` | Selectable values linked to attributes. |
| `products` | Parent product; must belong to same tenant. |
...
1. Select an existing product under the tenant.
2. Create one or more variants with SKU, optional barcode, display name, limits, and status.
3. Assign variant-defining attributes through selected attribute values when product has variants.
4. Validate SKU/barcode uniqueness and product relationship.
5. Save active variants for POS, cart, order, stock, return, exchange, and price list usage.

## Validation rules

- `sku` must be unique per tenant.
- `barcode` must be unique per tenant when present.
- `product_id` must belong to same tenant.
- `variant_signature` must be unique per product when present.
- Stock must remain outside the variant table.
