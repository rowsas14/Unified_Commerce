...
Stores receipt templates, generated receipt payloads, barcode values, print status, and print/reprint/email/download history.
...
| --- | --- | --- | --- |
| `receipt_templates` | `id` | tenant_id -> tenants.id; outlet_id nullable; document_type; format_type | Receipt/invoice templates. |
| `receipts` | `id` | tenant_id -> tenants.id; sale/order/return/exchange FK exactly one | Stored receipt output. |
| `receipt_print_logs` | `id` | tenant_id -> tenants.id; receipt_id; device_id | Print/reprint/email/download history. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/receipts/features/receipt-templates/feature-spec|Receipt Templates]] | Tenant/outlet-specific receipt/invoice templates by document type and format. | `receipt_templates` |
| [[07-modules/receipts/features/receipts/feature-spec|Receipts]] | Stored receipt/invoice output with frozen payload and barcode lookup value. | `receipts`, `sales`, `orders`, `returns`, `exchanges` |
| [[07-modules/receipts/features/receipt-print-logs/feature-spec|Receipt Print Logs]] | Print, reprint, email, and download history with failure reason. | `receipt_print_logs`, `receipts` |
