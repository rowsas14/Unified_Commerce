...
Defines tenant tax classes, tax rates, and effective tax-class-to-rate mappings used by catalog, POS, e-commerce orders, returns, refunds, and reports.
...
| --- | --- | --- | --- |
| `tax_classes` | `id` | tenant_id -> tenants.id; code unique per tenant | Tenant tax classification assigned to products. |
| `tax_rates` | `id` | tenant_id -> tenants.id; code; starts_at/ends_at | Effective tax rates. |
| `tax_class_rates` | `id` | tenant_id -> tenants.id; tax_class_id -> tax_classes.id; tax_rate_id -> tax_rates.id | Maps tax classes to rates. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/tax/features/tax-classes/feature-spec|Tax Classes]] | Tenant tax classification assigned to products. | `tax_classes` |
| [[07-modules/tax/features/tax-rates/feature-spec|Tax Rates]] | Effective VAT/GST/sales tax rates with country, start, end, and active status. | `tax_rates` |
| [[07-modules/tax/features/tax-class-rates/feature-spec|Tax Class Rates]] | Maps tax classes to rates with effective periods and overlap control. | `tax_class_rates`, `tax_classes`, `tax_rates` |
