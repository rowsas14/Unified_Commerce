...
Manages the tenant business account, outlets, outlet addresses, and tenant/outlet document numbering foundation for the production Unified Commerce platform.
...
| --- | --- | --- | --- |
| `tenants` | `id` | code UNIQUE; operating_mode; base_currency; default_timezone; status | Root business account for each customer tenant. |
| `outlets` | `id` | tenant_id -> tenants.id; code unique per tenant; outlet_type; status | Physical/logical stock and sales location. |
| `outlet_addresses` | `id` | tenant_id -> tenants.id; outlet_id -> outlets.id | Single operational address per outlet. |
| `document_sequences` | `id` | tenant_id -> tenants.id; outlet_id -> outlets.id nullable; document_type | Tenant/outlet-aware document number generator. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/tenant-management/features/tenants/feature-spec|Tenants]] | Root tenant/business account with operating mode, currency, locale, timezone, and lifecycle status. | `tenants` |
| [[07-modules/tenant-management/features/outlets/feature-spec|Outlets]] | Physical or logical sales/stock locations under a tenant. | `outlets`, `outlet_addresses` |
| [[07-modules/tenant-management/features/document-sequences/feature-spec|Document Sequences]] | Tenant/outlet-aware business document number generator for sale, order, return, exchange, receipt, transfer, purchase receipt, adjustment, and delivery. | `document_sequences` |
