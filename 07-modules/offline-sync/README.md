...
Owns backend-visible sync batches, sync items, typed sale/payment staging queues, conflicts, and technical sync audit logs for offline-first POS.
...
| --- | --- | --- | --- |
| `offline_sync_batches` | `id` | tenant_id -> tenants.id; outlet_id; device_id | One sync attempt from POS device. |
| `offline_sync_items` | `id` | tenant_id -> tenants.id; sync_batch_id; device_id; entity_type | Generic sync item queue. |
| `offline_sale_sync_queue` | `id` | tenant_id -> tenants.id; sync_item_id UNIQUE; device_id | Typed offline sale staging. |
| `offline_payment_sync_queue` | `id` | tenant_id -> tenants.id; sync_item_id UNIQUE; device_id | Typed offline payment staging. |
| `offline_sync_conflicts` | `id` | tenant_id -> tenants.id; sync_item_id; device_id | Conflict record. |
| `offline_sync_audit_logs` | `id` | tenant_id -> tenants.id; sync_batch_id; sync_item_id; device_id | Sync lifecycle technical audit. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/offline-sync/features/offline-sync-batches/feature-spec|Offline Sync Batches]] | One reconnect/sync attempt from a POS device. | `offline_sync_batches`, `pos_devices`, `outlets` |
| [[07-modules/offline-sync/features/offline-sync-items/feature-spec|Offline Sync Items]] | Generic sync item queue for offline-created records. | `offline_sync_items`, `offline_sync_batches`, `pos_devices` |
| [[07-modules/offline-sync/features/offline-sale-sync-queue/feature-spec|Offline Sale Sync Queue]] | Typed offline sale staging queue linked one-to-one with offline sync items. | `offline_sale_sync_queue`, `offline_sync_items` |
| [[07-modules/offline-sync/features/offline-payment-sync-queue/feature-spec|Offline Payment Sync Queue]] | Typed offline payment staging queue linked one-to-one with offline sync items. | `offline_payment_sync_queue`, `offline_sync_items` |
| [[07-modules/offline-sync/features/offline-sync-conflicts/feature-spec|Offline Sync Conflicts]] | Conflict record when offline sync cannot be accepted cleanly. | `offline_sync_conflicts`, `offline_sync_items` |
| [[07-modules/offline-sync/features/offline-sync-audit-logs/feature-spec|Offline Sync Audit Logs]] | Technical audit trail for sync lifecycle events. | `offline_sync_audit_logs`, `offline_sync_batches`, `offline_sync_items` |
