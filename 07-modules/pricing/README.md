...
Owns named price lists and variant prices by tenant, channel, currency, priority, effective dates, and optional outlet override.
...
| --- | --- | --- | --- |
| `price_lists` | `id` | tenant_id -> tenants.id; channel; currency; priority | Named channel price lists. |
| `price_list_items` | `id` | tenant_id -> tenants.id; price_list_id -> price_lists.id; variant_id -> product_variants.id; outlet_id nullable | Variant prices in price list. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/pricing/features/price-lists/feature-spec|Price Lists]] | Named tenant price lists for POS, e-commerce, or both channels. | `price_lists` |
| [[07-modules/pricing/features/price-list-items/feature-spec|Price List Items]] | Variant prices inside a price list with list price and optional sale price. | `price_list_items`, `product_variants`, `price_lists` |
| [[07-modules/pricing/features/outlet-price-overrides/feature-spec|Outlet Price Overrides]] | Outlet-specific override behavior through nullable outlet_id on price list items. | `price_list_items`, `outlets` |
