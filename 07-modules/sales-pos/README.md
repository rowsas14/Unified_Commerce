...
Owns POS cashier session control, scan/add/pay checkout, held sales, void/cancel behavior, cash movements, and sale line recording.
...
| --- | --- | --- | --- |
| `till_sessions` | `id` | tenant_id -> tenants.id; outlet_id; till_id; opened_by; closed_by; opened_device_id | Open/close session per till. |
| `cash_movement_types` | `id` | code UNIQUE; direction | Non-sale cash movement types. |
| `cash_movements` | `id` | tenant_id -> tenants.id; till_session_id; cash_movement_type_id; performed_by | Non-sale cash movement. |
| `cash_count_denominations` | `id` | tenant_id -> tenants.id; till_session_id | Denomination-level cash count. |
| `sales` | `id` | tenant_id -> tenants.id; outlet_id; till_session_id; customer_id; source_device_id | POS sale header. |
| `sale_lines` | `id` | tenant_id -> tenants.id; sale_id -> sales.id; variant_id -> product_variants.id | POS sale line items. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/sales-pos/features/till-sessions/feature-spec|Till Sessions]] | Open/close session per till with opening float, expected cash, counted cash, variance, and approval fields. | `till_sessions`, `tills`, `pos_devices` |
| [[07-modules/sales-pos/features/pos-checkout/feature-spec|POS Checkout]] | Barcode-first scan/add/pay cashier flow with active outlet, terminal, and session context. | `sales`, `sale_lines`, `till_sessions`, `product_variants` |
| [[07-modules/sales-pos/features/hold-recall-sale/feature-spec|Hold and Recall Sale]] | Uses POS sale status to hold and later recall unfinished sale context. | `sales`, `sale_lines` |
| [[07-modules/sales-pos/features/void-cancel-sale/feature-spec|Void and Cancel Sale]] | Controls voided and cancelled sale status behavior with audit and permission review. | `sales` |
| [[07-modules/sales-pos/features/cash-movements/feature-spec|Cash Movements]] | Non-sale cash in/out, safe drop, float add, and paid out movements within a till session. | `cash_movement_types`, `cash_movements` |
| [[07-modules/sales-pos/features/cash-count-denominations/feature-spec|Cash Count Denominations]] | Denomination-level till close count that supports cash reconciliation. | `cash_count_denominations`, `till_sessions` |
