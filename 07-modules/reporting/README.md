...
Provides daily read models for sales, payments, inventory, discounts, returns, and exchanges. These are reporting projections, not financial source-of-truth tables.
...
| --- | --- | --- | --- |
| `daily_sales_summaries` | `id` | tenant_id -> tenants.id; outlet_id nullable | Daily sales read model. |
| `daily_payment_summaries` | `id` | tenant_id -> tenants.id; payment_method_type_id | Daily payment read model. |
| `daily_inventory_summaries` | `id` | tenant_id -> tenants.id; outlet_id; variant_id | Daily inventory movement summary. |
| `daily_discount_return_summaries` | `id` | tenant_id -> tenants.id; outlet_id nullable | Daily discount/return/exchange summary. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/reporting/features/daily-sales-reporting/feature-spec|Daily Sales Reporting]] | Daily sales dashboard summary by tenant/outlet/channel/date. | `daily_sales_summaries` |
| [[07-modules/reporting/features/daily-payment-reporting/feature-spec|Daily Payment Reporting]] | Daily payment totals by method and scope. | `daily_payment_summaries`, `payment_method_types` |
| [[07-modules/reporting/features/daily-inventory-reporting/feature-spec|Daily Inventory Reporting]] | Daily inventory movement summary by outlet, variant, and business date. | `daily_inventory_summaries` |
| [[07-modules/reporting/features/daily-discount-return-reporting/feature-spec|Daily Discount and Return Reporting]] | Daily discount, return, and exchange summary by scope/channel/date. | `daily_discount_return_summaries` |
