...
Manages tenant-scoped POS and e-commerce customer profiles, auth account wrapper, auth identities, and customer addresses.
...
| --- | --- | --- | --- |
| `customers` | `id` | tenant_id -> tenants.id; normalized_email/phone tenant unique | Tenant-scoped customer profile. |
| `customer_auth_accounts` | `id` | tenant_id -> tenants.id; customer_id -> customers.id | Customer account wrapper. |
| `customer_auth_identities` | `id` | tenant_id -> tenants.id; account_id -> customer_auth_accounts.id | Login identities. |
| `customer_addresses` | `id` | tenant_id -> tenants.id; customer_id -> customers.id | Reusable customer addresses. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/customers/features/customer-profiles/feature-spec|Customer Profiles]] | Tenant-scoped customer profile shared by POS and online channel. | `customers` |
| [[07-modules/customers/features/customer-auth-accounts/feature-spec|Customer Auth Accounts]] | Customer account wrapper for online login. | `customer_auth_accounts`, `customers` |
| [[07-modules/customers/features/customer-auth-identities/feature-spec|Customer Auth Identities]] | Login identities for local, Google, and Apple providers. | `customer_auth_identities`, `customer_auth_accounts` |
| [[07-modules/customers/features/customer-addresses/feature-spec|Customer Addresses]] | Reusable tenant-scoped customer addresses for billing, shipping, and other address types. | `customer_addresses`, `customers` |
