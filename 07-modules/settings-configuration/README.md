---
title: Settings and Configuration Module
...
Stores tenant runtime settings, feature flags, and UI theme tokens by tenant, outlet, channel, or user scope where supported.
...
| --- | --- | --- | --- |
| `feature_flags` | `id` | tenant_id -> tenants.id; feature_id -> platform_features.id; outlet_id/user_id by scope | Tenant-side runtime feature configuration. |
| `tenant_settings` | `id` | tenant_id -> tenants.id; setting_key; scope; outlet_id/channel by scope | Tenant/outlet/channel settings. |
| `ui_themes` | `id` | tenant_id -> tenants.id; name; theme_tokens JSONB | Tenant UI theme tokens. |

## Feature map

| Feature | Purpose | Primary tables |
| --- | --- | --- |
| [[07-modules/settings-configuration/features/tenant-settings/feature-spec|Tenant Settings]] | Generic setting store for tenant/outlet/channel configuration. | `tenant_settings` |
| [[07-modules/settings-configuration/features/feature-flags/feature-spec|Feature Flags]] | Tenant-side runtime configuration for entitled platform features. | `feature_flags`, `platform_features` |
| [[07-modules/settings-configuration/features/ui-themes/feature-spec|UI Themes]] | Tenant UI theme tokens for colors, spacing, typography, and default theme selection. | `ui_themes` |
